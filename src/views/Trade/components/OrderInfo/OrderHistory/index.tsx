...
// redux
import { orderhistory, isLoading } from 'store/orderinfo/selectors'
...
const columns = ['Pair', 'Type', 'Side', 'Average', 'Price', 'Executed', 'Amount', 'Total']

function OrderHistory(props: any) {
  const classes = useStyles()
  const dispatch = useAppDispatch()
  const data = useAppSelector(orderhistory)
  ...
  const handleReset = () => {
    setStartTime('')
    setEndTime('')
    setDuringOption(During.ONE_DAY)
    const formData = new FormData()
